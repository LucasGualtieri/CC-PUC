...
public class TP02Q05 {
	...
	public static void main(String[] args) throws Exception {
		...
		int qtdDeRepeticoes = MyIO.readInt();
		while (qtdDeRepeticoes > 0) {
			input = MyIO.readLine().replaceAll("é", "\u00e9");
			char operacao = input.charAt(0);

			if (operacao == 'I' && posicaoArray < 100) {
				String nomeDoArquivo = new String();
				for (int c = 2; c < input.length(); c++) {
					nomeDoArquivo += input.charAt(c);
				}
				ArrayDePersonagens[posicaoArray] = new Personagem();
				ArrayDePersonagens[posicaoArray].Ler(nomeDoArquivo);
				posicaoArray++;
			}
			if (operacao == 'R' && posicaoArray >= 1)// checa se a próxima posição é igual a um, ou seja, se a pilha não
														// está vazia

			{
				posicaoArray -= 1;// ajusta para a última posição da pilha
				retiraPilha(posicaoArray, ArrayDePersonagens);
			}
			qtdDeRepeticoes--;
		}
		for (int i = 0; i < posicaoArray; i++) {
			System.out.print("[" + i + "] ");
			ArrayDePersonagens[i].Imprimir();
		}
	}
